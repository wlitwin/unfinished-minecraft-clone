...
bool ObjLoader::Load(std::istream& stream)
{
    ...
    while (stream.good() && !stream.eof())
    {
        ...
        if (type == "v")
        {
            ...
            m_verts.push_back(glm::vec3(r[0], r[1], r[2]));
        }
        else if (type == "vt")
        {
            ss >> t[0]; ss >> t[1];
            m_texs.push_back(glm::vec2(t[0], t[1]));
        }
        else if (type == "vn")
        {
            ss >> r[0]; ss >> r[1]; ss >> r[2]; 
            m_norms.push_back(glm::vec3(r[0], r[1], r[2]));
        }
        else if (type == "f")
        {
            string fdefs[3];    
...
bool ObjLoader::ToMesh(Mesh& mesh, unsigned char flags)
{
    ...
    for (size_t i = 0; i < m_faces.size(); ++i)
    {
        ...
        const glm::vec3& v1(m_verts[m_faces[i].val.vert[0]]);
        const glm::vec3& v2(m_verts[m_faces[i].val.vert[1]]);
        const glm::vec3& v3(m_verts[m_faces[i].val.vert[2]]);
